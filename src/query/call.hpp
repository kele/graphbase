...
public:
  explicit Call(const std::vector<std::shared_ptr<const IExpression>> &args)
    ...
    if (std::tuple_size<typename F::Args>::value != args.size()) {
      ...
      // TODO: std::string("Call to ") + F::name + " failed. Argument number
      // mismatch.");
    }
  }
  ...
  value::Value eval(std::shared_ptr<const Environment> env) const final {
    ...
    return evalImpl(
        env, std::move(t),
        std::make_index_sequence<std::tuple_size<typename F::Args>::value>());
  }
...
private:
  ...
  template <size_t... Is>
  value::Value evalImpl(std::shared_ptr<const Environment> env,
                        typename F::Args argsTuple,
